/**
 * CREDITS: https://github.com/codeoverflow-org/nodecg-io/blob/main/services/nodecg-io-discord-rpc/extension/discordRpcAuth.ts
 */
...
export async function createLoginData(
    ...
    if (!config.accessToken || !config.refreshToken || !config.expireTime) {
        ...
        const response = await nodeFetch("https://discord.com/api/oauth2/token", {
            method: "POST",
            headers: {
